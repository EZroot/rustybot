...
impl AudioManager {
    ...
    /// Create a new AudioManager ffmpeg source for a local file.
    pub async fn ffmpeg<P: AsRef<OsStr> + Send + Clone + Sync + 'static>(
        path: P,
        lazy: bool,
    ) -> Result<Self> {
        Self::new(FfmpegAudioManager { path }, lazy).await
    }

    /// Create a new AudioManager audioripper source.
    ///
    /// The cost of restarting and seeking will probably be *very* high:
    /// expect a pause if you seek backwards.
    pub async fn audioripper<P: AsRef<str> + Send + Clone + Sync + 'static>(
        uri: P,
        lazy: bool,
    ) -> Result<Self> {
        Self::new(AudioRipperAudioManager { uri }, lazy).await
    }

    /// Create a new AudioManager audioripper source, using the first result of a youtube search.
    ///
    /// The cost of restarting and seeking will probably be *very* high:
    /// expect a pause if you seek backwards.
    pub async fn audioripper_search(name: impl AsRef<str>, lazy: bool) -> Result<Self> {
        Self::audioripper(format!("ytsearch1:{}", name.as_ref()), lazy).await
    }
...
#[async_trait]
impl<P> Restart for FfmpegAudioManager<P>
where
    P: AsRef<OsStr> + Send + Sync,
{
    async fn call_restart(&mut self, time: Option<Duration>) -> Result<Input> {
...
struct AudioRipperAudioManager<P>
where
...
#[async_trait]
impl<P> Restart for AudioRipperAudioManager<P>
where
    P: AsRef<str> + Send + Sync,
{
    async fn call_restart(&mut self, time: Option<Duration>) -> Result<Input> {
        if let Some(time) = time {
            ...
            _audioripper(self.uri.as_ref(), &["-ss", &ts]).await
        } else {
            audioripper(self.uri.as_ref()).await
        }
    }

    async fn lazy_init(&mut self) -> Result<(Option<Metadata>, Codec, Container)> {
        _audioripper_metadata(self.uri.as_ref())
            .await
            .map(|m| (Some(m), Codec::FloatPcm, Container::Raw))
    }
}

impl From<AudioManager> for Input {
    fn from(mut src: AudioManager) -> Self {
        ...
        Input::new(stereo, Reader::AudioManager(src), kind, container, meta)
    }
}
    ...
    fn seek(&mut self, pos: SeekFrom) -> IoResult<u64> {
        ...
        match pos {
            ...
            End(_offset) => Err(IoError::new(
                ...
                "End point for AudioManagers is not known.",
            )),
