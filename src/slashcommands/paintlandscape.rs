...
pub async fn run(
    ...
    if let CommandDataOptionValue::String(attachment) = option {
        args_clone = format!("{} {}", attachment.clone(), "(wide-shot 1024x768), (--ar 3:2)");
    }
    ...
    generate_stable_diffuse_image(&args_clone, 512, 1024,50, 1, true).await.unwrap() // 944x944 891k for paragon
}
